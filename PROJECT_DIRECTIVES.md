...
- **`canva_designs`**: `id`, `project_id`, `kind` (`design` | `carousel`), `format`, `title`, `design_url`, `thumb_url` (foto principale usata), `slides`, `created_at`
  ...
  **Elimina design (2026-09-09)**: il 🗑 in `CreatedDesignsPanel` chiama `delete_design` che, oltre a cancellare la riga, sposta il design nel **Cestino di Canva** via `trashCanvaDesign()` (`canva-lib.js`) — Canva Connect non ha delete-design, quindi: crea cartella → `POST /v1/folders/move` → `DELETE /v1/folders/{id}`. Richiede lo scope OAuth **`folder:write`** (aggiunto in `canva-auth.js`: utenti già collegati devono riconnettere Canva). Best-effort: se manca lo scope la riga sparisce comunque e la UI avvisa. `history.js` importa `canva-lib.js` per questo.
